...
class Attention(nn.Module):
    ...
    def forward(self, x, kv=None, H=None, W=None):
        B, N, C = x.shape
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, attn_type=0.0,
                 H=0, W=0):
        super().__init__()
        self.norm1 = norm_layer(dim)
        # self.attn = Attention(
        #     dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        attn_func = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
                              attn_drop=attn_drop, proj_drop=drop)
        self.attn = GRC_Self_Attention(
            dim, attention_func=Attention, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, H=H, W=W, cache_ratio=attn_type
        ) if attn_type > 0 else attn_func
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000,
                 ...
                 local_feature=False, sie_xishu=1.0, attn_type=0.0, mem=0, lam=0.0, mem_q=False):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                attn_type=attn_type, H=self.patch_embed.num_y, W=self.patch_embed.num_x)
    ...
    def forward(self, x, cam_label=None, view_label=None):
        x, attn_list = self.forward_features(x, cam_label, view_label)
        return x, attn_list
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            elif self.attn_type > 0.0 and 'attn' in k:
                k = k.replace('attn.', 'attn.attn_self_func.')
